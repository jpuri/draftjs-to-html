...
function getInlineStyleSectionMarkup(block, styleSection, customColors) {
  var stylePropertySections = getInlineStyleSections(block, ['COLOR', 'BGCOLOR', 'FONTSIZE', 'FONTFAMILY', 'UNDERLINE'], styleSection.start, styleSection.end);
...
function getSectionMarkup(block, entityMap, section, customEntityTransform, customColors) {
  ...
  var inlineStyleSections = getInlineStyleSections(block, ['BOLD', 'ITALIC', 'STRIKETHROUGH', 'CODE', 'SUPERSCRIPT', 'SUBSCRIPT'], section.start, section.end);
      ...
      blocks.forEach(function (block) {
        if ((0, _list.isList)(block.type)) {
          ...
          if (listBlocks.length > 0) {
            var listHtml = (0, _list.getListMarkup)(listBlocks, entityMap, hashtagConfig, directional, customEntityTransform, customColors);
            html.push(listHtml);
            listBlocks = [];
          }
...
*/
function getListMarkup(listBlocks, entityMap, hashtagConfig, directional, customEntityTransform, customColors) {
  ...
  listBlocks.forEach(function (block) {
    ...
    if (!previousBlock) {
      ...
      if (nestedListBlock && nestedListBlock.length > 0) {
        listHtml.push(getListMarkup(nestedListBlock, entityMap, hashtagConfig, directional, customEntityTransform, customColors));
        nestedListBlock = [];
      }
    } else {
      nestedBlock = true;
